...
/**
 * Final Working Author Search
 *
 * This script searches all posts for author signatures without using select parameter
 */
...
async function finalWorkingAuthorSearch() {
  console.log('🔍 Final Working Author Search')
  console.log('===============================\n')

  const baseUrl = process.env.PAYLOAD_PUBLIC_SERVER_URL || 'http://localhost:3000'

  try {
    // Fetch all posts without select parameter
    console.log(`📡 Fetching posts from ${baseUrl}/api/posts...`)

    const response = await fetch(`${baseUrl}/api/posts?limit=1000&depth=0`)
    ...
    posts.forEach((post: any) => {
      ...
      if (content.length > 100) {
        ...
        // Progress indicator
        if (postsWithValidContent % 50 === 0) {
          console.log(`📝 Processed ${postsWithValidContent} posts with valid content...`)
        }
      }
    })
    ...
    console.log(`Posts with content: ${postsWithContent}`)
    console.log(`Posts with valid content: ${postsWithValidContent}`)
...
finalWorkingAuthorSearch().catch(console.error)
