  ...
  for (const element of elements) {
    ...
    if (paragraphMatch) {
      ...
      if (
        ...
        (textChildren.length === 1 && textChildren[0].text.trim() === '')
      ) {
        continue
      }
...
async function importWordPressProper() {
  const payload = await getPayload({ config: configPromise })

  console.log('Starting PROPER WordPress import...')
  ...
  for (const post of posts) {
    try {
      ...
      const originalSlug = post['wp:post_name']?.[0] || ''
      const content = post['content:encoded']?.[0] || ''
      const publishedAt = post['wp:post_date']?.[0] || new Date().toISOString()

      // Generate proper slug from title instead of using WordPress slug
      const properSlug = generateSlug(title)
          ...
          if (attachment) {
            ...
            if (attachmentUrl) {
              try {
                ...
                const filename = `${properSlug}-featured${ext}`

                // Upload to Payload (will use local storage since Vercel Blob is disabled)
                ...
                console.log(`   ⚠️  Failed to upload image: ${imageError}`)
      ...
      const verify = await payload.find({
        collection: 'posts',
        where: { id: { equals: createdPost.id } },
        limit: 1,
      })

      if (verify.docs.length === 0) {
        console.error(`   ⚠️  WARNING: Post was created but can't be found in database!`)
      }
    } catch (error) {
...
// Run the import
importWordPressProper().catch((error) => {
