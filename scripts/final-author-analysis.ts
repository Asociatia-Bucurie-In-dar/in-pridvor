...
/**
 * Final Author Analysis with Correct Patterns
 * 
...
// Final signature patterns based on actual content analysis
const SIGNATURE_PATTERNS = [
  ...
  { 
    pattern: /Pr\.\s+([A-Z][a-z]+(?:\-[A-Z][a-z]+)?\s+[A-Z][a-z]+)(?=\s|$|Fotografie|Din|Traducere|Acuarelă)/i, 
    type: 'end',
    description: 'Pr. Nume Prenume',
    confidence: 0.95
  },
  { 
    pattern: /Dr\.\s+([A-Z][a-z]+\s+[A-Z][a-z]+)(?=\s|$|Fotografie|Din|Traducere)/i, 
    type: 'end',
    description: 'Dr. Nume Prenume',
    confidence: 0.95
  },
  { 
    pattern: /Ierom\.\s+([A-Z][a-z]+(?:\s+\([^)]+\))?)(?=\s|$|Fotografie|Din|Traducere)/i, 
    type: 'end',
    description: 'Ierom. Nume (with optional parentheses)',
    confidence: 0.95
  },
  { 
    pattern: /monah\s+([A-Z][a-z]+\s+[A-Z][a-z]+)(?=\s|$|Fotografie|Din|Traducere)/i, 
    type: 'end',
    description: 'monah Nume Prenume',
    confidence: 0.95
  },
  { 
    pattern: /Traducere\s+de\s+([A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+\([^)]+\))?)(?=\s|$)/i, 
    type: 'end',
    description: 'Traducere de Nume Prenume (with optional parentheses)',
    confidence: 0.9
  },
  { 
    pattern: /Din\s+„.*"\.?\s*de\s+([A-Z][a-z]+\s+[A-Z][a-z]+)(?=\s|$)/i, 
    type: 'end',
    description: 'Din "..." de Nume Prenume',
    confidence: 0.9
  },
  { 
    pattern: /Din\s+volumul.*de\s+([A-Z][a-z]+\s+[A-Z][a-z]+)(?=\s|$)/i, 
    type: 'end',
    description: 'Din volumul... de Nume Prenume',
    confidence: 0.9
  },
  { 
    pattern: /Din\s+predica.*Traducere\s+de\s+([A-Z][a-z]+\s+[A-Z][a-z]+)(?=\s|$)/i, 
    type: 'end',
    description: 'Din predica... Traducere de Nume Prenume',
    confidence: 0.9
  },
  // Simple name patterns at the end (last resort)
  { 
    pattern: /([A-Z][a-z]+\s+[A-Z][a-z]+)(?=\s*$|Fotografie|Din|Traducere|Acuarelă|canvas)/i, 
    type: 'end',
    description: 'Simple name at end',
    confidence: 0.7
  },
]
  ...
  for (const { pattern, type, description, confidence: baseConfidence = 0.5 } of SIGNATURE_PATTERNS) {
    ...
    if (matches) {
      ...
      if (name && name.length > 2 && name.length < 50) {
        ...
        if (isRomanianName(name)) confidence += 0.1
...
async function finalAuthorAnalysis() {
  console.log('🔍 Final Author Signature Analysis')
  console.log('==================================\n')
  ...
  try {
    ...
    console.log(`High confidence authors (>80%): ${Object.values(authorStats).filter(s => s.confidence > 0.8).length}`)
    ...
    const highConfidenceAuthors = Object.entries(authorStats)
      .filter(([, stats]) => stats.confidence > 0.8 && stats.count >= 1)
      .sort(([,a], [,b]) => b.count - a.count)
    ...
    // Generate the creation script data
    console.log('\n🚀 Next Steps:')
    console.log('=' .repeat(50))
    console.log('1. Run the author creation script with these high-confidence authors')
    console.log('2. Update posts to use the new authors instead of "Demo Author"')
    console.log(`3. Found ${highConfidenceAuthors.length} authors ready for creation`)
...
finalAuthorAnalysis().catch(console.error)
